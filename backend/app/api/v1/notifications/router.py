...
from app.api.v1.dependencies import notification_owner
...
async def get_notification_endpoint(
    notification: NotificationModel = Depends(notification_owner),
):
...
async def unread_count_endpoint(current_user: UserModel = Depends(get_current_user)):
...
async def counts_endpoint(current_user: UserModel = Depends(get_current_user)):

    counts = await get_notification_counts(current_user.id)
