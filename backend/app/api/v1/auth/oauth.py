"""OAuth service for Google and GitHub authenticationHandles authorization code exchange, user verification, and JWT creation"""
...
class OAuthState:
    ...
    """State parameter for CSRF protection - simple in-memory storage"""
    ...
    _state_ttl_seconds = 600  # 10 minute TTL
...
@classmethod
    ...
    def generate(cls) -> str:
        ...
        cls._states[state] = {"created_at": now}
...
@classmethod
    ...
    def validate(cls, state: str) -> bool:
        ...
        created_at = cls._states[state].get("created_at")
        ...
        if created_at:
            ...
            if age_seconds > cls._state_ttl_seconds:
...
@classmethod
    ...
    def consume(cls, state: str) -> bool:
...
class GitHubOAuth:
    ...
    async def get_user_info(access_token: str) -> Optional[OAuthUser]:
        ...
        return None
async def find_or_create_user(oauth_user: OAuthUser) -> Optional[UserModel]:
    ...
    existing_user = await db["users"].find_one({"email": oauth_user.email})
